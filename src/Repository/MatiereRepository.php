...
class MatiereRepository extends ServiceEntityRepository
{
    ...
    public function add(Matiere $entity, bool $flush = false): void
    {
    ...
    public function getIdMatiere($idMatiere)
    {
        $connexion = $this->_em->getConnection();

        $requete = "SELECT * FROM `matiere` WHERE classse_id = $idMatiere";

        $resultat = $connexion->executeQuery($requete);

        $data = $resultat->fetchAssociative();
