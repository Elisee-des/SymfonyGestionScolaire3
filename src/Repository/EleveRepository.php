...
class EleveRepository extends ServiceEntityRepository
{
    ...
    public function getEleves($idAnnee, $idParent)
    {
        $connexion = $this->_em->getConnection();
        $requete = "SELECT * FROM `eleve` WHERE user_id=$idParent AND annee_id=$idAnnee";
        $resultat = $connexion->executeQuery($requete);
        $data = $resultat->fetchAllAssociative();
        return $data;
    }

    public function findEleves($idClasse)
    {
        $connexion = $this->_em->getConnection();
        $requete = "SELECT * FROM `eleve` WHERE classe_id = $idClasse";
        $resultat = $connexion->executeQuery($requete);
        $data = $resultat->fetchAllAssociative();
        return $data;
    }
